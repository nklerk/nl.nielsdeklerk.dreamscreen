...
Client.prototype.handleReceivedPacket = function(received, rinfo) {
  ...
  if (received[0] != 0xfc || length < 6) {
    // must have atleast 1 byte of payload  //validates length
    if (this.debug) {
  ...
  if (!light) {
    console.log(`Adding light to database: "${senderIp}"`);
    if (this.debug) {
    ...
    if (commandReceived == 0x010a && (!isResponseRequested && isResponse)) {
      //0x010A, received Get State payload
      let productId = payload[payload.length - 1];
      if (productId == 1 || productId == 2) {
        //Dreamscreen HD, DreamScreen 4K
        ...
        console.log(`Trying basic config.`);
        light = new Light({
          client: this,
    ...
    if (commandReceived == 0x0103) {
    ...
    } else if (commandReceived == 0x010a) {
      //0x010A, received Get State payload
      let productId = payload[payload.length - 1];
      ...
      if (productId == 1 || productId == 2) {
        //Dreamscreen HD, DreamScreen 4K specific payload indexing
        ...
        //SideKick specific payload indexing
      ...
      if (lightUpdated) {
        ...
        console.log(`Updating state of light "${senderIp}"`);
...
Client.prototype.light = function(identifier) {
  ...
  if (typeof identifier !== "string") {
    throw new TypeError(`DreamScreen Client light expects identifier for DreamScreen light to be a string but got VALUE:${identifier} AS:${typeof identifier}`);
  }

  // There is no ip or id longer than 45 chars
  if (identifier.length > 45) {
    //ipv4 address is max 15 characters? eg 111.111.111.111
    return false;
  }
